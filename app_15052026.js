/* =========================================================
   PADRONIZADOR DE EXTRATOS GT — app.js v3.1
   Correções principais:
   - Inicialização segura com DOMContentLoaded
   - Status muda imediatamente ao clicar em Processar
   - Priorização da aba "Lançamentos"
   - Leitura de extrato Itaú:
     Data | Lançamento | Razão Social | CPF/CNPJ | Valor | Saldo
   - Fallback por posição de coluna
   - Tratamento de valores positivos/negativos padrão brasileiro
   - Geração RAW_EXTRATO e PIVOT_MENSAL
========================================================= */
...
/* =========================================================
   PARSER ITAÚ
   Suporta:
   1) Cabeçalho detectado automaticamente
   2) Fallback por posição:
      A Data | B Lançamento | C Razão Social | D CPF/CNPJ | E Valor | F Saldo
========================================================= */
...
function findHeaderRow(rows){
  for(let i = 0; i < Math.min(rows.length, 150); i++){
    ...
    let valorIdx = findIndex(row, [
      "valoroperacao",
      "valordaoperacao",
      "valor"
    ]);

    const saldoIdx = findIndex(row, [
      "saldo",
      "saldoatualizado"
    ]);

    if(valorIdx === saldoIdx && valorIdx >= 0){
      valorIdx = -1;
    }
